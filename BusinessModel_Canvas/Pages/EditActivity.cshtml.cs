...
namespace BusinessModel_Canvas.Pages
{
    public class EditActivityModel : PageModel
    {
        ...
        public EditActivityModel(Canvas_Context context)
        {
            _context = context;
        }

        public void OnGet(Guid Activity)
        {
            ResourceID = Activity;
            var resource = _context.Resources.Where(s => s.Id == ResourceID && s.IsEvent == true).FirstOrDefault();
            if (resource == null)
